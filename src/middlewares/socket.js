...
export const createSocketMiddleware = (socket) => store => {
  ...
  socket.on('connect', async () => {
    ...
    const { socket: { sessionListMap } } = getState();
    const restoreMap = await socketService.restoreSessionFromLocal(sessionListMap);
    const user = await  Storage.get('murray/user');
    await dispatch(createAction('auth/modify')({ userId: user.data._id, field: 'socketId', value: socket.id }));
