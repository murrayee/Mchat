...
class ChatWindow extends Component {
    ...
    static defaultProps = {
        ...
        onMessagePress: (type, index, content) => {
            console.log(type, index, content)
        },
        onMessageLongPress: (type, index, content) => {
            console.log('longPress', type)
        },
        ...
        voiceSpeakIcon: <Image source={require('../source/image/speak.png')}
    ...
    _scrollToBottom(even) {
        ...
        if (!scrollProperties.contentLength) {
            return
        }
    ...
    render() {
        ...
        return (
            <Animated.View style={{flex: 1}}>
                <FlatList
                    style={[{flex: 1, backgroundColor: '#f1f1f1'}, this.props.chatWindowStyle]}
                    ...
                    renderItem={({item}) => {
                        return (
                            <View style={{transform: [{scale: inverted ? -1 : 1}],borderWidth:1,borderColor:'red'}}>
